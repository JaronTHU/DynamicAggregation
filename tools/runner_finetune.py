...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if hasattr(config, 'train_transforms'):
        train_transforms = [PointcloudResample(config.npoints)] if config.train_transforms.resample else [PointcloudNaiveSample(config.npoints)]
        ...
        train_transforms = Compose([
            PointcloudResample(config.npoints),
            PointcloudScaleAndTranslate()
        ])
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (_, _, data) in enumerate(tqdm(train_dataloader)):
            num_iter += 1
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(test_dataloader)):
            ...
            for _ in tqdm(range(times)):
                ...
                points = pointnet2_utils.gather_operation(points_raw.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()  # (B, N, 3)
