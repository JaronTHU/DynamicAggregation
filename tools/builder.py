...
def dataset_builder(args, config):
    ...
    shuffle = config.others.subset.startswith('train')
    if args.distributed:
...
# only save tunable parameters for some Module with efficient_state_dict
...
def save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, logger = None):
    if args.local_rank == 0:
        ...
        # if args.distributed, base_model is ddp model  else dp model, both have to add '.module'
        save_model = base_model.module
        if hasattr(save_model, 'efficient_state_dict'):
            save_dict['base_model'] = save_model.efficient_state_dict()
            save_dict['backbone_path'] = args.ckpts
        else:
            save_dict['base_model'] = save_model.state_dict()
